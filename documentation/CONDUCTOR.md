...
A **conductor** is one long-lived agent session whose whole job is to supervise other sessions. Today, conductor runtimes include Claude Code, GitHub Copilot CLI, and Codex. You do not code in it. You do not read files with it. You talk to it (from your terminal, from Telegram, from Slack, from anywhere you have a bot) and it delegates the work to child sessions, watches what they do, and reports back.
...
- A long-lived AI CLI process (`claude`, `copilot`, or `codex`) pinned inside a named tmux session managed by agent-deck.
...
- **It survives your terminal.** Close iTerm, reboot, come back a day later. The tmux session is still there, the conductor process is still there, and the conductor knows where it left off.
...
Requirements: agent-deck installed, a project path in mind, the runtime you want to use (`claude`, `copilot`, or `codex`) installed locally, and (optional) a Telegram bot token.
...
Or start one on GitHub Copilot CLI:

```bash
agent-deck conductor setup my-copilot-conductor \
  --agent copilot \
  --model claude-sonnet-4.6 \
  --allow-all \
  --description "Supervises work in ~/projects/foo via Copilot"
```
...
The tmux session spins up and the selected runtime boots inside it. Claude and Copilot conductors use `CLAUDE.md`; Codex conductors use `AGENTS.md`. Copilot conductors are bootstrapped with an initial prompt that tells Copilot to read both the per-conductor `./CLAUDE.md` and the shared `../CLAUDE.md`.
...
The startup checklist the conductor runtime reads on every session start or resume. Claude and Copilot conductors use `CLAUDE.md`; Codex conductors use `AGENTS.md`. The file is generated by `conductor setup` from a template and is customizable with `--instructions-md`. It tells the conductor what its scope is, what agents live in its group, and what the reading order of the other files is.
